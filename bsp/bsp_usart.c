...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
  if(huart->Instance==USART10)
    USART10_TX_CPLT_HOOK(huart->hdmatx);
}
...
/***************USART10函数***************/
static void USART10_StartRx(void)
{
  if(HAL_UARTEx_ReceiveToIdle_DMA(
       &huart10, USART10_RxBuf0, (uint16_t)USART10_BUF_SIZE) == HAL_OK)
  {
    /* IDLE/TC are sufficient for framed traffic; HT would restart early. */
...
{
  (void)huart;
  (void)CircBuf_Push(&USART10_RxCBuf, USART10_RxBuf0, Size);
  USART10_StartRx();
}
...
static void USART10_StartNextTx(void)
{
  unsigned int index;
  unsigned int used;
  uint16_t send_len;

  if(HAL_DMA_STATE_READY != HAL_DMA_GetState(huart10.hdmatx))
    return;

  used = CircBuf_GetUsedSize(&USART10_TxCBuf);
  send_len = (uint16_t)MIN(used, sizeof(USART10_TxBuf));
  if(send_len == 0u)
    return;

  /* Keep bytes queued until HAL has accepted the DMA transfer. */
  for(index = 0u; index < send_len; ++index)
    USART10_TxBuf[index] = CircBuf_At(&USART10_TxCBuf, index);
  if(HAL_UART_Transmit_DMA(&huart10, USART10_TxBuf, send_len) == HAL_OK)
    CircBuf_Drop(&USART10_TxCBuf, send_len);
}
...
unsigned int USART10_Send(uint8_t *data, unsigned short len)
{
  ...
  /* Concurrent tasks may queue only complete protocol frames. */
  if(CircBuf_GetAvalaibleSize(&USART10_TxCBuf) < len)
  {
  ...
  result = CircBuf_Push(&USART10_TxCBuf, data, len);
  USART10_StartNextTx();
